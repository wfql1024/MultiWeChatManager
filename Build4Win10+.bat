...
.venv38\Scripts\pyinstaller --name="微信多开管理器By38" --windowed --icon=external_res\SunnyMultiWxMng.ico ^
--add-data="external_res;external_res" ^
--version-file=.meta\version.txt --noconfirm --hidden-import=comtypes.stream Main.py

:: 检查打包是否成功
if exist "dist\微信多开管理器By38\微信多开管理器By38.exe" (
    echo 正式版打包成功
    rename "dist\微信多开管理器By38\微信多开管理器By38.exe" "微信多开管理器.exe"
) else (
    echo 正式版打包失败
    goto end
)
...
:: 复制快捷方式创建脚本到打包文件夹
copy "点我创建快捷方式.bat" "dist\微信多开管理器By38\"
