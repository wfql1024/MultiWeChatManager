...
from public_class.widget_frameworks import ActionableClassicTable
...
class ClassicRowUI:
    def __init__(self, result):
        ...
        self.classic_table_class["login"] = AccLoginClassicTable(
                self, "login", "已登录：", self.btn_dict["auto_quit_btn"])
...
class AccLoginClassicTable(ActionableClassicTable, ABC):
    class AccLoginRow(ActionableClassicTable.ItemRow, ABC):
    ...
    def create_rows(self, item):
        ...
        print(f"渲染{item}...")
        # 创建列表实例
        row = AccLoginClassicTable.AccLoginRow(self, item, table_tag)
        self.rows[item] = row
