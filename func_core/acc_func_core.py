...
class AccOperatorCore:
    @staticmethod
    def _safe_get_screen_size() -> Tuple[int, int]:
        """获取屏幕尺寸"""
    ...
    def _open_acc_return_hwnd_and_pid(cls, sw, acc, all_excluded_hwnds):
        ...
        login_rules_dicts = SwInfoFuncCore.get_sw_wnd_class_matching_dicts(sw, WndType.LOGIN)
        # 调取配置,打开登录窗口 *******************************************************************
        if AccInfoFuncCore.is_acc_coexist(sw, acc):
            # 共存程序账号: 尝试查找有无同名打开进程, 有则需要去除互斥体, 共存账号无需替换配置文件
            print(f"[OK]{acc}是共存号,无需登录配置")
            sw_proc, sub_proc = SwOperatorCore.open_sw(sw, exe=acc)
        else:
            ...
            if success:
                ...
                return None, None
            sw_proc, sub_proc = SwOperatorCore.open_sw(sw)
    ...
    def _login_accounts(cls, login_dict: Dict[str, List]):
        ...
        if acc_cnt is None or acc_cnt == 0:
            return
        # 计算窗口排列位置 ===================================================================
        screen_size = cls._safe_get_screen_size()
        ...
        for sw, accounts in login_dict.items():
            ...
            if multirun_mode == MultirunMode.FREELY_MULTIRUN:
                ...
                SwInfoFuncCore.record_sw_pid_mutex_dict_when_start_login(sw, False)
            else:
    ...
    def open_sw_and_ask(cls, sw, account):
        ...
        root = root_class.root
        login_ui = root_class.login_ui
        if AccInfoFuncCore.is_acc_coexist(sw, account):
        ...
        if messagebox.askyesno(
            ...
            # 手动登录原始程序
            sw_hwnd, msg = Sw(sw).open_sw_and_return_hwnd()
            if sw_hwnd:
                time.sleep(1)
    ...
    def silent_get_and_config(cls, sw):
        ...
        if curr_config_acc is not None:
            ...
            if AccInfoFuncCore.get_sw_acc_login_cfg_status(sw, curr_config_acc) == CfgStatus.NO_CFG.value:
...
"""
        ...
        replace_cmd_list = []
        for addr in config_addresses:
            origin_cfg_path = SwInfoFuncCore.resolve_sw_path(sw, addr).replace("/", "\\")
            acc_cfg_path = os.path.join(os.path.dirname(origin_cfg_path), f"{acc}_{os.path.basename(origin_cfg_path)}")
            acc_cfg_path = acc_cfg_path.replace("/", "\\")
...
class AccInfoFuncCore:
    ...
    @staticmethod
    def get_sw_acc_login_cfg_status(sw, account) -> str:
        """
        通过账号的配置状态
        :param sw: 选择的软件标签
        :param account: 账号
        ...
        one_origin_cfg_path = (SwInfoFuncCore.resolve_sw_path(sw, config_addresses[0]).replace("\\", "/"))
    ...
    def trims_other_sw(trim_values, other_sw):
        ...
        return other_cut_map, now_l, now_r
    ...
    def get_sw_accounts_login_status(cls, sw):
        ...
        all_acc_list = Sw(sw).get_existed_accounts()
    ...
    def get_main_hwnd_of_accounts(cls, sw, acc_list) -> Tuple[Optional[dict], str]:
        ...
        wnd_class_matching_dicts = SwInfoFuncCore.get_sw_wnd_class_matching_dicts(sw, WndType.MAIN)
    ...
    @classmethod
    def auto_bind_main_wnd_to_accounts_in_sw(cls, sw, acc_list):
