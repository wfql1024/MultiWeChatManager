...
def quit_selected_accounts(sw, accounts_selected):
    ...
    if messagebox.askokcancel("提示",
                              f"确认退登：\n{accounts_to_quit_str}？"):
        try:
            quited_accounts = _quit_accounts(sw, accounts_selected)
...
def _quit_accounts(sw, accounts):
    ...
    for account in accounts:
        try:
            ...
            display_name = FuncAccInfo.get_acc_origin_display_name(sw, account)
            cleaned_display_name = StringUtils.clean_texts(display_name)
...
class FuncAccInfo:
    ...
    def get_acc_wrapped_display_name(sw, account) -> str:
        ...
        return StringUtils.balanced_wrap_text(
            FuncAccInfo.get_acc_origin_display_name(sw, account),
            10
        )
    ...
    def silent_get_and_config(sw):
        ...
        if len(accounts_need_to_get_avatar) > 0:
            changed = FuncAccInfo._silent_get_avatar_url(sw, accounts_need_to_get_avatar, data_dir)
            if changed is True:
                need_to_notice = True

        # 3. 对待获取昵称的账号尝试遍历获取
        if len(accounts_need_to_get_nickname) > 0:
            changed = FuncAccInfo._silent_get_nickname(sw, accounts_need_to_get_nickname, data_dir)
            if changed is True:
                need_to_notice = True
