...
class Test(TestCase):
    ...
    def test_move_window_to_foreground_topleft(self):
        ...
        # print(move_window_by_classname("注册表编辑器"))
        print(move_window_to_foreground_topleft(33820986))
    ...
    def test_get_all_handles(self):
        handles = handle_utils.pywinhandle_find_handles_by_pids_and_handle_name_wildcards([23488])
        print(handles)
    ...
    def test_get_unique_memory_maps(self):
        import psutil
        from collections import defaultdict

        def get_unique_memory_maps(pid_list):
            """
            返回每个 pid 独有的 memory_maps
            key: pid
            value: [pmmap_ext, ...]
            """
            pid_maps = {}
            ...
            for pid in pid_list:
                ...
                print(f"{pid}: 总长度为{len(maps)}")
                pid_maps[pid] = maps
                pid_key_sets[pid] = {
                    m.path for m in maps
                }

            # 2. 构造所有 pid 的 key 全集
            all_keys = defaultdict(set)
            for pid, keys in pid_key_sets.items():
                for k in keys:
                    all_keys[k].add(pid)
            ...
            for pid, maps in pid_maps.items():
                unique_maps = [
                    m for m in maps
                    if len(all_keys[m.path]) == 1
                ]
                print(f"{pid}: 独长度为{len(unique_maps)}")
                result[pid] = unique_maps

            return result

        res_dict = get_unique_memory_maps([30172, 22300])
        print(res_dict)
